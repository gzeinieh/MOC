...
angular.module('missioncontrolApp').factory('Auth', function Auth($http, User, $cookies, $q) {
  /**
  ...
  return {
    ...
    /**
     * Create a new user WITHOUT logging in as that user
     *
     * @param  {Object}   user     - user info
     * @param  {Function} callback - optional, function(error, user)
     * @return {Promise}
    ...
    createNewUser: function createNewUser(user, callback) {
      return User.save(user, function (data) {
        return safeCb(callback)(null, user);
      }, (function (err) {
        this.logout();
