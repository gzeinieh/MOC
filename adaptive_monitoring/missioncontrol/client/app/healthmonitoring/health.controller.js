  ...
  .controller('HealthCtrl', function ($scope, $http) {
     ...
     .then((response) => {
      ...
      $scope.$watch('selectedMission', function(){
        ...
        $http.get('/api/satellites/mission/' +$scope.selectedMission._id)
        .then((response) => {
          console.log(response);
    ...
    $scope.checkMission = function() {
               ...
               $http.get('/api/sensors/satellite/' +$scope.selectedSatellite._id + '?limitTo=' + $scope.range)
                   .then((response) => {
                   console.log(response);
                   if (Array.isArray(response.data)){
    ...
    $scope.updateThresholds = function() {
        ...
        for (var i=0; i<= $scope.sensors.length; i++){
            ...
            $http.put('/api/sensors/'+$scope.sensors[i]._id,{"max_threshold":$scope.sensors[i].max_threshold, "min_threshold":$scope.sensors[i].min_threshold})
            .then((response) => {
              console.log(response);
            ...
            $http.post('/api/thresholds',{"max_threshold":$scope.sensors[i].max_threshold, "min_threshold":$scope.sensors[i].min_threshold, "SensorId":$scope.sensors[i]._id, "updatedAt": $scope.getDateTime()})
            .then((response) => {
              console.log(response);
            });    
    ...
    $scope.checkMission2 = function() {
            ...
            for (var i=0; i<= $scope.sensors.length; i++){
                 $http.get('/api/sensors/sensor_model_data/'+ $scope.sensors[i].Sensor_type.model.toLowerCase() + '/sensor/' + $scope.sensors[i]._id)
                 .then((response) => {
                    console.log(response);
